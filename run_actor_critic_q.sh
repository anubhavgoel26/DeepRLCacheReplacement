...
for nn_type in shallow deep attention; do
    for lr in 0.00001 0.00005 0.0001 0.0005; do
        ...
        CUDA_VISIBLE_DEVICES=$count python3 train.py -c 50 -n $nn_type --lr $lr -a ActorCriticQ > outputs/ActorCriticQ_50_${nn_type}_${lr}.log 2>&1 & 
        count=$((count+1))
    done
done
...
#     python train.py -c $cache_size -n $BEST_NN --lr $BEST_LR -a ActorCriticQ > outputs/ActorCriticQ_${cache_size}_${BEST_NN}_${BEST_LR}.log
# done